//This macro measures neurite length and additional parameters from fluorescence images of high-density neuron cultures. The cultures should be stained for at least one neurite marker and one nuclear marker.
...
// this function segments neurites from the image
	...
	run("Duplicate...", "title=[edge mask]");
	run("Duplicate...", "title=[high intensity mask]");

	//process high intensity mask
	selectWindow("high intensity mask");
	setAutoThreshold("Moments dark");
	setOption("BlackBackground", true);
	run("Convert to Mask");

	//process edge mask
	selectWindow("edge mask");
	run("Unsharp Mask...", "radius=12.5 mask=0.60");
	setAutoThreshold("Moments dark");
	setOption("BlackBackground", true);
	run("Convert to Mask");
	...
	//process LoG mask
	selectWindow(imageWindow);
	run("FeatureJ Laplacian", "compute smoothing=0.25");
	setAutoThreshold("Moments dark");
	run("Convert to Mask");
	run("Invert");
	rename("LoG mask");
	selectWindow(imageWindow); //close windows not needed
	close();
	
	//prepare neurite mask
	imageCalculator("Add create", "high intensity mask","edge mask");
	imageCalculator("Add create", "Result of high intensity mask","LoG mask");
	imageCalculator("Subtract create", "Result of Result of high intensity mask","nuclei_mask");
	selectImage("Result of Result of Result of high intensity mask");
	rename("neurite_mask");
	
	//remove speckles
	selectWindow("neurite_mask");
	run("Analyze Particles...", "size=0.20-Infinity show=Masks");
	run("Invert LUT");
	
	//prepare neuronal network mask
	imageCalculator("Add create", "nuclei_mask","neurite_mask");
...
// this function calculates various neurite properties (total length, number and area) given the image of a soma and dendrite
	...
	run("Duplicate...", "title=[nuclei_mask-skeleton]");
	run("Skeletonize");
	...
	imageCalculator("AND create", "nuclei_mask-attachments","nuclei_mask-skeleton");
	...
	selectImage("nuclei_mask-skeleton");
	...
	selectImage("neurite_mask");
	run("Analyze Particles...", "size=0.2-Infinity circularity=0-1.00 summarize");

	//Measure fragments area
	selectImage("neurite_mask");
	run("Analyze Particles...", "size=0.2-25 circularity=0.2-1.00 show=[Bare Outlines] summarize");
	...
	if (saveNeuriteMask) {
		//Save neurite masks;
		run("Merge Channels...", "c2=[nuclei_mask-skeleton] c3=[nuclei_mask] c4=[neurite attachment points mask] create keep");
